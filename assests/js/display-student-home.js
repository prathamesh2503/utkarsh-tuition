export function displayStudentDataOnHome() {
  ...
  const displyHomeStudentData = document.querySelector(
    "#home-student-container"
  );

  const showToHomeStudentData =
    JSON.parse(localStorage.getItem("showToHomeStudentData")) || [];

  if (
    showToHomeStudentData.length === 0 ||
    showToHomeStudentData === null ||
    showToHomeStudentData === undefined
  ) {
