let addedStudentData = JSON.parse(
  localStorage.getItem("addedStudentData") || "[]"
);
...
addStudentSubmitButton.addEventListener("submit", function (e) {
  ...
  reader.onload = function (event) {
    ...
    if (
      addedStudentData.find(
        (s) =>
          s.studentName === studentName &&
          s.studentClass === studentClass &&
          s.achievementYear === achievementYear &&
          s.studentPercentage === studentPercentage
      )
    ) {
...
function displayStudentData() {
  ...
  if (
    addedStudentData === "" ||
    addedStudentData === null ||
    addedStudentData.length === 0
  ) {
    studentContainer.innerHTML = `
    ...
    addedStudentData.forEach((student) => {
      ...
      deleteStudent.forEach((btn) => {
        btn.addEventListener("click", function () {
          ...
          if (showToHomeStudentData.find((s) => s.id === studentId)) {
            ...
            const foundStudent = addedStudentData.find(
              (s) => s.id === studentId
            );

            if (foundStudent) {
              addedStudentData = addedStudentData.filter(
                (student) => student.id !== foundStudent.id
              );
              localStorage.setItem(
                "addedStudentData",
                JSON.stringify(addedStudentData)
              );
            } else {
      ...
      showToHomeButton.forEach((btn) => {
        btn.addEventListener("click", function () {
          ...
          if (showToHomeStudentData.find((s) => s.id === studentId)) {
            ...
            localStorage.setItem(
              "showToHomeStudentData",
              JSON.stringify(showToHomeStudentData)
            );
            showToHomeStudentData = JSON.parse(
              localStorage.getItem("showToHomeStudentData")
            );
          }
        });
      });
      ...
      hideToHomeButton.forEach((btn) => {
        btn.addEventListener("click", function () {
          ...
          const foundStudent = showToHomeStudentData.find(
            (s) => s.id === studentId
          );
          if (foundStudent) {
            showToHomeStudentData = showToHomeStudentData.filter(
              (student) => student.id !== foundStudent.id
            );
            localStorage.setItem(
              "showToHomeStudentData",
  ...
  function showStudentDataInTable() {
    ...
    const total = addedStudentData.reduce(
      (sum, student) => sum + parseFloat(student.studentPercentage),
      0
    );
    const average =
      addedStudentData.length > 0
        ? (total / addedStudentData.length).toFixed(2)
        : 0;
    studentDataTableBody.innerHTML += `
        <tr>
